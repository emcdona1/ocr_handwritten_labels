...
class ClassificationApp:
    def __init__(self, **kw):
        ...
        self.suggest_engine = root.suggestEngine
...
def InitializeConfiguration(root):
    ...
    root.parallelProcess = True if root.configParser.get('IMAGE_PROCESSOR', 'parallelProcess').lower() in ['t', 'y',
                                                                                                           'true',
                                                                                                           'yes',
                                                                                                           '1'] else False
    root.parallelProcessThreadCount = int(root.configParser.get('IMAGE_PROCESSOR', 'parallelProcessThreadCount'))
    root.sortItemsByBarCode = True if root.configParser.get('TAG_LIST', 'sortItemsByBarCode').lower() in ['t', 'y',
                                                                                                          'true', 'yes',
                                                                                                          '1'] else False
