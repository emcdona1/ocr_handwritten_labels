...
* `/`
    ...
    * `crop_images_of_words.py`
    * `gather_ocr_data_from_cloud_platforms.py`
    * `prep_comparison_data.py`
    * `Configuration-plain.cfg`
    * `requirements.txt`
* `/imageprocessor/`
    * Contains the ImageProcessor and ImageAnnotator classes,
      for handling, parsing, and visualizing OCR data. 
    * `image_annotator.py`
    * `image_processor.py`
* `/labelcorpus/`
...
* `/nameresolution/`
    * (not in use) Contains files for fuzzy text matching/error correction,
...
### Excluded directories which are generated by scripts
* `/imageprocessor_objects/`
    * Contains all pickle files created by ImageProcessor
* `/test_results/`
    * Various processed CSV files and annotated images will be stored here.
