...
def main():
    image_processor = setup_google_cloud()
    ...
    # list_of_images = os.listdir('label_images')
    list_of_images = ['C0601155F_label.jpg']
    for one_image in list_of_images:
        ...
        # gcv_response = load_gcv_response_from_pickle(pickle_folder, 'C0611253F.pickle')
        print('Successful GCV query.')
        pickle_an_object(gcv_response, image_barcode, pickle_folder)
                ...
                for w_idx, word in enumerate(paragraph.words):
                    ...
                    for s_idx, symbol in enumerate(word.symbols):
                        ...
                        word_filename = generate_zooniverse_images(image_to_draw_on, image_barcode, word, symbol,
                                                   b_idx, p_idx, w_idx, s_idx, '' if s_idx == 0 else word_filename)
...
def generate_neural_net_symbol_images(image, image_barcode, symbol, b_idx, p_idx, w_idx, s_idx):
    ...
    image_filename = save_boxed_word_image_to_file(image_of_one_cropped_symbol, image_folder_nn,
...
def generate_zooniverse_images(image, image_barcode, word, symbol, b_idx, p_idx, w_idx, s_idx, word_filename: str) -> str:
    ...
    if s_idx == 0:
        ...
        word_filename = save_boxed_word_image_to_file(image_with_word_box, image_folder_zooniverse,
    ...
    image_with_one_symbol_boxed_cropped_to_word = crop_image(image_with_one_symbol_boxed,
                                                             word.bounding_box)
    image_filename = save_boxed_word_image_to_file(image_with_one_symbol_boxed_cropped_to_word,
...
def setup_google_cloud() -> ip.ImageProcessor:
...
def pickle_an_object(obj_to_pickle, base_image_name_for_folders: str, save_directory: str) -> None:
    ...
    filename = base_image_name_for_folders + '.pickle'
...
def save_boxed_word_image_to_file(image_to_save: np.ndarray, save_folder: str, image_type_prefix: str,
...
def save_symbol_image_to_file(save_folder: str, image_barcode: str, image_to_save: np.ndarray,
                              b_idx: int, p_idx: int, w_idx: int, s_idx: int) -> str:
    """ Assumes save_folder exists."""
    filename = image_barcode + '-one_symbol-' + str(b_idx) + '-para' + str(p_idx) \
               + '-word' + str(w_idx) + '-sym' + str(s_idx) + '.jpg'
    file_path = os.path.join(save_folder, filename)
    cv2.imwrite(file_path, image_to_save)
    return filename
